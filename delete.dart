  ...
  Future<void>delete()async{
    ...
    for(int k=1; k<=i; k++)
    {
      ...
      if(int.parse(v.toString())==int.parse(searchcontroller1.text.toString()))
      {
        ...
        for(int j=1; j<=9; j++)
        {
          l.add(sheet.rows[k][j]?.value?.toString());
        }
        list.add(l);
        Navigator.push(context, MaterialPageRoute(builder: (context)=>DeleteConfirm(p: list)));
      }
    }
    if(flag!=1)
    {
                      ...
                      Row(
                        children: <Widget>[
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                Text("Remove Book:",style: TextStyle(fontSize: MediaQuery.of(context).size.height/25, fontWeight: FontWeight.w400),),
                              ],
                            ),
                          ),
                          SizedBox(width: MediaQuery.of(context).size.width/20),
